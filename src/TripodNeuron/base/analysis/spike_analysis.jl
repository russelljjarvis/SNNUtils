...
function get_spikes(voltage::Array{Float64,3}; column=1)
    """
    ...
    if column == 1
        ...
        for n in 1:size(voltage,1)
            spikes[n,:] .= [ v == AdEx.AP_membrane for v in voltage[n,1,:]]
        end
    end
    if column == 3
...
get_spikes(voltage::Array{Float64,1})::Array{Bool,1} = [v == AdEx.AP_membrane for v in voltage[:]]

function get_spikes(voltage::Array{Float64,2})
    spikes = falses(size(voltage,2),size(voltage,1))
    for n in 1:size(voltage,1)
        spikes[:,n] .= [ v == AdEx.AP_membrane for v in voltage[n,:]]
    end
    return spikes
end

##
get_spike_times(voltage::Array{Float64,2}) = findall(x->x==AdEx.AP_membrane, get_spikes(voltage))
...
get_spike_times(voltage::Array{Float64,1}) = findall(get_spikes(voltage))

##
get_spike_rate(v::Array{Float64,2})::Float64 = float(sum(v[1,:].== AdEx.AP_membrane)/length(v[1,:])*10000)
get_spike_rate(v::Array{Float64,1}) = sum(v.== AdEx.AP_membrane)/length(v)*10000
get_spike_rate(v::Array{Float64,3}) = [sum(v[i,1,:].== AdEx.AP_membrane)/length(v[i,1,:])*10000 for i in 1:size(v,1)]
...
##
function get_isi(input::Union{Matrix{Float64}, Vector{Float64}, BitArray{1}})
    if isa(input,Array{Float64,2})
        return diff(get_spike_times(input[1,:]))
    elseif isa(input,BitArray{1})
        return diff(get_spike_times(input))
    elseif isa(input,Array{Float64,1})
        return diff(get_spike_times(input))
    end
end

function CV_isi(intervals::Vector{Float64})
    # input::Union{Matrix{Float64}, Vector{Float64}, BitArray{1}})
    _cv = sqrt(var(intervals)/mean(intervals)^2)
    return isnan(_cv) ? 0. : _cv
end

function CV_voltage(voltage::Vector{Float64})
...
function average_potential(membrane::Array{Float64,2}, time_step::Float64 ; slide_offset::Float64 = 20, window::Float64 = 50)
    ### the time of each bin is 0.1 ms.
