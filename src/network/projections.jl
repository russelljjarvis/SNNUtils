...
function ExternalInput(population, connection_prob=0.05; cells=nothing, rate=1000Hz, strength=10, inputs_nodes=10)
	if isa(population,SNN.Tripod)
        return TripodExternalInput(population, connection_prob; cells=cells, rate=rate, strength=strength, inputs_nodes=inputs_nodes, n_dend=2)
	end
...
function TripodExternalInput(population, connection_prob=0.05; cells=nothing, rate=1000Hz, strength=10, inputs_nodes=10, n_dend=2)
	if isa(population,SNN.Tripod)
		@assert n_dend <=2 "Tripod has maximum 2 dendrites"
	end
	input_pop = SNN.Poisson(N = inputs_nodes, param = SNN.PoissonParameter(rate = rate))
	synapses = Vector{SNN.AbstractSparseSynapse}() 
	for n in 1:n_dend # loop over all dendrites
		W = zeros(population.N, inputs_nodes)
        if isnothing(cells)
            for i in eachrow(W) #loop over all postsynaptic neurons
                if rand(Bernoulli(connection_prob)) # make connection with Bernoulli prob
        ...
        s = SNN.SynapseTripod(input_pop, population, Symbol("d$n"), :exc, w=W)
		push!(synapses, s)
	end
	return (syn=synapses, pop=input_pop)
end
