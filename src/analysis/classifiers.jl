...
#
    ...
    for (n, batch) in enumerate(spikes)
        features[:, 1+(n-1)*n_samples:n*n_samples] .= _spike_to_features(batch, input_duration, batch_duration)
    end
...
"""
 function _spike_to_features(spikes::Union{Vector, Tuple},
...
function states_to_features(states::Vector{NNStates}; average_points=true)
    ...
    for (n, state) in enumerate(states)
        ...
        features[:,pos ] .= _state_to_features(state.mem, state.cur; average=average_points)
    end
    return features, n_neurons
end

function _state_to_features(membranes::Matrix{Float32},
    ...
    if average
        ...
        return feature = vcat(reshape(membranes, (x*N_POINTS,n)),
...
function MultiLogReg(X::SubArray{Float32,2,Matrix{Float32}},labels::Array{Int64}; λ=0.5f0::Float32, test_ratio=0.7f0)
...
function score_features(;features::Matrix{Float32}, n_neurons::Int64, seq::SeqEncoding, idxs::Vector{Int}, save_path::String, feats_set::NamedTuple)
    ...
    @views word_neurons = sort(collect(Set(vcat(seq.populations[sort(collect(Set(word_labels)))]...))))
    @views ph_neurons = sort(collect(Set(vcat(seq.populations[sort(collect(Set(ph_labels)))]...))))
