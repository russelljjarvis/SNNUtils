    ...
    # for index in 1:length(spikes)
        ...
        while input_time < last_tt
            ...
            m =  round(Int,input + previous_inputs)
		 	rates[1:net.tripod,m] = get_spike_rate(exc, interval=(input_time, input_time+stim.duration))
            for n in eachindex(wn_labels)
                words_activity[n,m] = mean(get_spike_rate(exc[seq.populations[wn_labels[n]]],interval=(input_time,input_time+stim.duration)))
            end
            for n in eachindex(pn_labels)
                phonemes_activity[n,m] = mean(get_spike_rate(exc[seq.populations[pn_labels[n]]],interval=(input_time,input_time+stim.duration)))
            end
...
function score_rate(;path, category)
	try
		if category=="phonemes"
			...
			myseq = read(fid["seq"])
			return mean(length(get_words(myseq)) .+argmax.(eachcol(ph)) .== myseq[2,1:12000])
...
function score_spikes(;path::String, tt0::Int=-1)
    ...
    idxs = rescale_with_tt0_ratio(store.data,stim,tt0)
	score_features(;features=features, n_neurons=n_neurons,
	 				seq=seq,
					idxs=idxs,
					save_path=save_path,
					feats_set=(; :cv=>n_neurons+1:2*n_neurons, :rate=>1:n_neurons, :all=> 1:2*n_neurons)
					)
end
...
function target_pop_rate(interval::Int64, seq::Encoding, network_state::Array{Float32,3}, stim::StimParams, symbol::Int)
        ...
        target_interval = 1+round(Int, stim.duration/dt*(interval-1)):round(Int, stim.duration/dt*interval)
