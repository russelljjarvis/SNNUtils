...
get_spike_times(spikes::Union{BitArray{2},Array{Bool,2}}) = map(x->get_spike_times(x), eachrow(spikes))
...
get_spike_rate(v::Array{Float32,2})::Float32 = float(sum(v[1,:].>-30)/length(v[1,:])*10000)
get_spike_rate(v::Array{Float32,1}) = sum(v.>-30)/length(v)*10000
get_spike_rate(v::Array{Float32,3}) = [sum(v[i,1,:].>-30)/length(v[i,1,:])*10000 for i in 1:size(v,1)]
get_spike_rate(spikes::Union{BitArray{1},Array{Bool,1}}) = sum(spikes)/length(spikes)*10000

function get_spike_rate(spike_times::Spiketimes ; interval::Tuple{Float32,Float32}=(0f0,0f0))
    rate = Vector{Float32}()
